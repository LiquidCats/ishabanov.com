...
class PostsApiV1Test extends TestCase
{
    ...
    {
        $data = PostModel::factory()->make();

        $response = $this->postJson(route('admin.api.posts.store'), [
            ...$data->toArray(),
            'tags' => $this->tags->random(3)->toArray(),
        ...
        $this->assertEquals(51, PostModel::query()->count());
    }
    ...
    public function test_should_update_post(): void
    {
        ...
        $tags = $this->tags
            ->random(3)
            ->map(
                fn (TagModel $t) => $t->only(['id', 'name', 'slug'])
            )
            ->toArray();
        $data = [
        ...
        foreach ($attributes as $attribute) {
            $this->assertEquals(
                $data[$attribute],
                $response->json("data.{$attribute}"), "{$attribute} is not equal"
            );
        }


        self::assertEquals(Collection::make($data['tags'])->sortBy('id')->toArray(), $tags);
    }
    ...
    public function test_should_delete_post(): void
    {
        ...
        $this->assertNull(PostModel::query()->find($post->getKey()));
    }
