...
const useFilesState = defineStore<'admin.files', State, any, Actions>('admin.files', {
    ...
    actions: {
        ...
        async upload(fileToUpload?: FileToUpload): Promise<void> {
            ...
            let data: FileResource[] = []

            try {
                this.status.filesUploading = [...toUpload]
                const response  = await FileApi.upload(toUpload)
        ...
        async paginate(page: number = 1): Promise<void> {
            try {
                ...
                const {data, meta, links} = await FileApi.getFilesList(page)
        ...
        async remove(hash: string): Promise<void> {
            try {
                ...
                await FileApi.remove(hash)

                this.status.filesDeleting = this.status
                    .filesDeleting
                    .filter((h) => h !== hash)

                this.items = this.items.filter((f: FileResource) => f.hash !== hash)
            } catch (e) {
                const notifications = useNotificationState()
